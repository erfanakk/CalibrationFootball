...
import argparse, cv2, torch, pathlib, logging as LOGGER
...
def process_one(path: pathlib.Path, out_dir: pathlib.Path, save_masks: bool,
                model, device):
    ...
    mask_lines, mask_goals = build_line_masks(keypoints, (h_img, w_img), conf_th=0.1)

    # Overlay
    overlay = img_bgr.copy()
    overlay[mask_lines > 0] = (202 ,47,43)  # yellow
    overlay[mask_goals > 0] = (202 ,47,43)    # red
    ...
    if save_masks:
        cv2.imwrite(str(out_dir / f"{stem}_lines.png"), mask_lines)
        cv2.imwrite(str(out_dir / f"{stem}_goals.png"), mask_goals)
...
#     parser.add_argument("--frame", default=r"images\frame_125.jpg", help="input broadcast frame")
...
def main() -> None:
    p = argparse.ArgumentParser("Key‑point detection + line segmenter (batch)")
    p.add_argument("--frame", help="single image path (optional)")
    p.add_argument("--dir",default=r"final_images", help="folder of images to process")
    p.add_argument("--out", default="overlay.png", help="(single mode) output PNG")
    p.add_argument("--out_dir", default="out_eagle", help="(batch mode) folder for results")
    p.add_argument("--save_masks", action="store_true",
                   help="also save 'lines.png'/'goals.png' (or per-image)")
    args = p.parse_args()
    ...
    # single-image mode
    if args.frame and not args.dir:
        img_bgr = cv2.imread(args.frame)
        if img_bgr is None:
            raise FileNotFoundError(args.frame)
        h_img, w_img = img_bgr.shape[:2]
        ...
        if device == "cuda":
            torch.cuda.synchronize()
        mask_lines, mask_goals = build_line_masks(keypoints, (h_img, w_img), conf_th=0.01)
        overlay = img_bgr.copy()
        overlay[mask_lines > 0] = (0, 255, 255)
        overlay[mask_goals > 0] = (0, 0, 255)
        cv2.imwrite(args.out, overlay)
        if args.save_masks:
            cv2.imwrite("lines.png", mask_lines)
            cv2.imwrite("goals.png", mask_goals)
    ...
    # batch mode
    if not args.dir:
        raise SystemExit("Provide either --frame or --dir")

    in_dir = pathlib.Path(args.dir)
    ...
    LOGGER.info(f"Processing {len(files)} images from {in_dir} -> {out_dir}")
    for fp in tqdm(files):
        try: 
            process_one(fp, out_dir, args.save_masks, model, device)
        except:
            continue
